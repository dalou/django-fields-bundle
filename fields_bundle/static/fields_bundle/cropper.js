...
$(document).ready(function(INIT, CHANGE_IMAGE, REPLACE, APPLY)
{

    APPLY = function($input, $input_cropped, src, $img)
    {
        ...
        $img.cropper({
          ...
          // preview: ".img-preview",
            ...
            console.log(coords)
            //$img.cropper('setCropBoxData', coords);
        });
    }
    ...
    REPLACE = function($input, $input_cropped, src)
    {
        $img = $input_cropped.parent().find('img')
        $img.cropper('destroy')
        $container = $input_cropped.parent()
        $container.find('img').remove()
        $img = $('<img />').css({ width: '100%', maxHeight:400 });
    ...
    CHANGE_IMAGE = function($input, $input_cropped, files, delete_input){
        ...
        else
        {
            ...
            reader.onload = (function(newFile) {
                return function(e) {
                    REPLACE($input, $input_cropped, e.target.result);
    ...
    $('[data-image-cropped]').each(function() {
        INIT($(this));
    ...
    setTimeout(function() {
                console.log($('[data-image-cropped]'))
        $('[data-image-cropped]').each(function() {
            INIT($(this));
        });
    },500);
    ...
    INIT = function($input_cropped) {
        if(!$input_cropped[0].is_croppable) {
            ...
            $input = $('#id_'+$input_cropped.data('image-cropped'));
                console.log($input_cropped.val())
